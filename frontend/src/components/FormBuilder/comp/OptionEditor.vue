...
<script setup>
    ...
    watch(local_q_type, (newType) => {
        localOptions.value.forEach((opt) => {
            if (!opt.meta_data) opt.meta_data = {};
            opt.meta_data.type_of_rules = newType;
        });
        emitOptions();
    });
    ...
    function addOption() {
        localOptions.value.push({
            ...
            meta_data: {
                rules: [],
                type_of_rules: local_q_type.value, // ⬅️ هنا
                is_locked: false, // ⬅️ هنا
            },
        });
        emitOptions();
    }
